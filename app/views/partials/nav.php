...
<nav class="navbar navbar-expand-lg helppy-nav">
  <div class="container-fluid helppy-nav-container">
    ...
    <!-- Desktop-only: Abonohu pill next to brand -->
    <?php if (Auth::check() && (Auth::role() === 'provider' || Auth::role() === 'admin')): ?>
      <a class="nav-abonohu-btn d-none d-lg-inline-flex" href="<?= e(CONFIG['base_url']) ?>/subscribe">
        <i class="bi bi-stars"></i>
        <span>Abonohu</span>
      </a>
    <?php endif; ?>

    <!-- Hamburger — pushes to right on mobile -->
    ...
    <div class="collapse navbar-collapse" id="navmenu">
      ...
      <?php if (Auth::check()): ?>
        ...
        <!-- ── MOBILE PROFILE HEADER (top of menu, mobile only) ── -->
        <div class="nav-menu-profile d-lg-none">
          <div class="d-flex align-items-center gap-3">
            <div class="nav-menu-avatar"><?= e(mb_strtoupper(mb_substr((string)$__u['name'], 0, 1))) ?></div>
            <div>
              <div class="fw-bold text-white" style="font-size:15px"><?= e($__u['name']) ?></div>
              <div style="font-size:12px;color:rgba(255,255,255,.6)"><?= e($__u['email'] ?? '') ?></div>
              <span class="nav-role-pill"><?= e(Auth::role() ?? '') ?></span>
            </div>
          </div>
          <div class="nav-menu-profile-links">
            <a class="nav-menu-profile-link" href="<?= e($__myDashUrl) ?>">
              <i class="bi bi-person-circle"></i> <?= e($__myDashLabel) ?>
            </a>
            <?php if (Auth::role() === 'provider' || Auth::role() === 'admin'): ?>
              <a class="nav-menu-profile-link nav-menu-abonohu" href="<?= e(CONFIG['base_url']) ?>/subscribe">
                <i class="bi bi-stars"></i> Abonohu
              </a>
            <?php endif; ?>
          </div>
        </div>
        <hr class="nav-menu-divider d-lg-none">
      ...
      <ul class="navbar-nav mx-auto align-items-lg-center gap-lg-1">
        <li class="nav-item">
          <a class="nav-link text-white" href="<?= e(CONFIG['base_url']) ?>/search">
            <i class="bi bi-grid d-lg-none me-2"></i>Shërbimet
          </a>
        </li>
        <li class="nav-item">
          <a class="nav-link text-white" href="<?= e(CONFIG['base_url']) ?>/si-funksionon">
            <i class="bi bi-play-circle d-lg-none me-2"></i>Si funksionon
          </a>
        </li>
        <li class="nav-item">
          <a class="nav-link text-white" href="<?= e(CONFIG['base_url']) ?>/rreth-nesh">
            <i class="bi bi-info-circle d-lg-none me-2"></i>Rreth nesh
          </a>
        </li>
        <li class="nav-item">
          <a class="nav-link text-white" href="<?= e(CONFIG['base_url']) ?>/kontakt">
            <i class="bi bi-envelope d-lg-none me-2"></i>Kontakt
          </a>
        </li>
      </ul>

      <!-- ── RIGHT-SIDE: desktop auth + mobile actions ── -->
        ...
        <?php if (Auth::check()): ?>
          ...
          <li class="nav-item">
            <button type="button" class="nav-link text-white nav-icon-link theme-toggle-btn"
                    data-theme-toggle title="Ndrysho temë">
          ...
          <!-- Mobile logout at bottom of menu -->
          <li class="nav-item d-lg-none mt-1">
            <hr class="nav-menu-divider">
            <form method="post" action="<?= e(CONFIG['base_url']) ?>/logout" class="m-0">
              <input type="hidden" name="_csrf" value="<?= e(Request::csrfToken()) ?>">
              <button class="nav-link text-danger w-100 text-start nav-link-icon border-0 bg-transparent" type="submit">
                <i class="bi bi-box-arrow-right"></i> <span>Dilni</span>
              </button>
            </form>
          </li>
